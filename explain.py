...
def lcs(G, start, goal):
	frontier = []	# heap
	heapq.heappush(frontier, (0,0,0, [start])) # seed with start
	while frontier:	
		# (523, 'Broome', 'Derby', '07:17', '08:57', 60)
		# (60, )
		(pcost, flighttime, pstop, path) = heapq.heappop(frontier)
		state1 = path[-1] # last element of path
		if state1 == goal: # we're done!
			embed()
			return path[1::2] # start at index 1, hop 2 steps (why?)
		
		for nstate in G[state1]: # city; destcity(nstate)
			for flight in G[state1][nstate]: # (cost, startmin, stopmin)
				(cost, start, stop) = G[state1][nstate][flight]

				# only add if start time is later than stop time
				if start > pstop: # in case 0 pstop is 0
					total_cost = pcost + cost # straight up dijkstra
					if flighttime == 0:
						newtime = (stop-start)
					else:
